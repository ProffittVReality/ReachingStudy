			...
			if (Skeleton_Root) {
				...
				if (GUILayout.Button ("Prepare Bones") ) {
					...
					if (!TargetScript.DirectMotion) {
						GameObject targetSkeletonPrefab = AssetDatabase.LoadAssetAtPath("Assets/PerceptionNeuron/Resources/TargetMotionSkeleton.prefab", typeof(GameObject)) as GameObject;
					...
					if (AddColliders) {
						...
						GameObject colliders = AssetDatabase.LoadAssetAtPath("Assets/PerceptionNeuron/Resources/DefaultColliderSetup.prefab", typeof(GameObject)) as GameObject;
