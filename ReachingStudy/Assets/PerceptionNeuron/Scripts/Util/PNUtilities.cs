...
public class PNUtilities : MonoBehaviour {
	...
	public static void CreateBoneLines(Transform[] target, string prefix, float startSize, float endSize, Color color, int rQPlus ) {
		...
		for (int i=0; i < target.Length; i++) {			
			if (target[i] != null) { 
				...
				Material newMat;
				if (rQPlus == 0 ) {
					newMat = UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/PerceptionNeuron/Resources/BoneLinesMaterial_0.mat", typeof(Material)) as Material;
				} else {
					newMat = UnityEditor.AssetDatabase.LoadAssetAtPath("Assets/PerceptionNeuron/Resources/BoneLinesMaterial_1.mat", typeof(Material)) as Material;
				}
				...
				newMat.color = color;
				newBV.GetComponent<Renderer>().sharedMaterial = newMat;
	...
	public static void ToggleColliders(GameObject target) {
		...
		foreach (CapsuleCollider cc in BoneCapsuleColliders) {
			cc.GetComponent<Renderer>().enabled = !cc.GetComponent<Renderer>().enabled;
		}
		foreach (BoxCollider bc in BoneBoxColliders) {
			bc.GetComponent<Renderer>().enabled = !bc.GetComponent<Renderer>().enabled;
		}
	}
	...
	public static void SetColliderRenderer(GameObject target, bool state) {
		...
		foreach (CapsuleCollider cc in BoneCapsuleColliders) {
			cc.GetComponent<Renderer>().enabled = state;
		}
		foreach (BoxCollider bc in BoneBoxColliders) {
			bc.GetComponent<Renderer>().enabled = state;
		}
	}
