...
public class PNSkeletonMotion : MonoBehaviour {
	...
	void Start () {		
		if (AutoSettings) {
			...
			if (Skeleton [1].GetComponent<Rigidbody>() != null) {
	...
	void FixedUpdate () {
		...
		if (!DirectMotion) { // only do physics movement stuff if we're not applying data values directly
			if (_dataParser.IsConnected && UseUpdateByPhysics) {
				if (Skeleton[0].GetComponent<Rigidbody>() == null ) {
				...
				for (int i=0; i<Skeleton.Length; i++) {
					if (Skeleton[i] != null) {
						UpdateTransformRigidbody(Skeleton[i].GetComponent<Rigidbody>(), TargetSkeleton[i].position, TargetSkeleton[i].rotation);
					}
				}
			}
		}
